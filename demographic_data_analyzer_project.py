...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = (bachelors_count / total_count).round(decimals = 1)
   ...
   high_salary_df = education_salary_df.loc[(slice(None), '>50K'), :]
    ...
    higher_education = education_salary_df.loc[['Bachelors', 'Master', 'Doctorate']].sum()
    lower_education = education_salary_df - higher_education

    # percentage with salary >50K
    higher_education_rich_count = education_salary_df.loc[['Bachelors', 'Masters', 'Doctorate']].sum()
    lower_education_rich_count = education_salary_df.sum() - higher_education_rich_count

    higher_education_rich = float(higher_education_rich_count / higher_education*100).round(decimals = 1) 
